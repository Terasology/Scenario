...
public class LogicTree extends Tree<LogicTreeValue> {
        ...
        if (expanded) {
            if (value.getEntity() != null) {
                switch (value.getValueType()) {
                    //If it is a name(event/conditional/action) then it needs to find the matching entity from the 
                    // trigger
                    case EVENT_NAME:
    ...
    public boolean acceptsChild(Tree<LogicTreeValue> child) {
        ...
        switch (this.getValue().getValueType()) {
            ...
            case ACTION_NAME:
                ...
                break;
            case TRIGGER:
                if (child.getValue().getValueType() == LogicTreeValue.Type.EVENT_NAME || //Can't move 
                        // event/cond/action names
    ...
    public void addChild(int index, Tree<LogicTreeValue> child) { //Should only send if moving entities on UI view
        hubToolScreen.getEntity().send(new LogicTreeMoveEntityEvent(this.getValue().getEntity(),
                child.getValue().getEntity(), child.getValue().getValueType(), index));
    }
